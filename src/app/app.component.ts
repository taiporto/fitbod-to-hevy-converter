...
export class AppComponent {
      ...
      if (droppedFile.fileEntry.isFile) {
        ...
        fileEntry.file((file: File) => {
          ...
          reader.onload = () => {
            ...
            console.log({ fitbodData, hevyData });
            const csv = this.JSONtoCSV(hevyData);
            const file = new File([csv], 'FitBodToHevyConvertedFile.csv', {
              type: 'csv/text',
            });

            console.log(this.JSONtoCSV(hevyData));
  ...
  fitBodToHevy(fitBodData: Fitbod[]): Hevy[] {
    ...
    const hevyData = fitBodData.map((entry) => {
      const exerciseKey = `${entry.date.substring(0, 10)}_${
        entry.exercise_name
      }`; // Unique key for each exercise and date
      ...
      if (exerciseMappings[entry.exercise_name]) {
        entry.exercise_name = exerciseMappings[entry.exercise_name];
      }

      return {
        Date: entry.date.substring(0, 19),
        'Workout Name':
          'Workout on: ' + moment(entry.date).format('YYYY-MM-DD'),
        'Exercise Name': entry.exercise_name,
        'Set Order': setOrder,
